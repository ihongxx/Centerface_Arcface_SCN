...
class CenterFace(object):
    def __init__(self, landmarks=True):
        ...
        if self.landmarks:
            self.net = cv2.dnn.readNetFromONNX('./checkpoints/centerface.onnx')
        else:
    ...
    def __call__(self, img, height, width, threshold=0.5):
        """
        __call__方法，使得类实例对象可以像调用普通函数那样调用
        :param img:原始图片
        :param height:原始图片高度
        :param width:原始图片宽度
        :param threshold:
    ...
    def inference_opencv(self, img, threshold):
        #cv2.dnn.blobFromImage是对图像进行预处理操作，size为神经网络真正的输入大小，saclefactor图像缩放大小，mean减去的均值，swapRB将BGR转成RGB
        blob = cv2.dnn.blobFromImage(img, scalefactor=1.0, size=(self.img_w_new, self.img_h_new), mean=0, swapRB=True, crop=False)
        self.net.setInput(blob)
        begin = datetime.datetime.now()
        if self.landmarks:
            heatmap, scale, offset, lms = self.net.forward(["537", "538", "539", "540"])  # 获得前向传播中最后输出的四个值
        else:
